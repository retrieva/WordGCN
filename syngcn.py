...
class SynGCN(Model):
    def getBatches(self, shuffle=True):
        ...
        while True:
            ...
            eph_ovr = self.lib.getBatch(self.edges_addr, self.wrds_addr, self.negs_addr, self.samp_addr, self.elen_addr,
                                        self.wlen_addr, self.p.win_size, self.p.num_neg, self.p.batch_size,
                                        ctypes.c_float(self.p.sample))
            if eph_ovr == 1:
                break
            yield {'edges': self.edges, 'wrds': self.wrds, 'negs': self.negs, 'sample': self.samp,
                   'elen': self.elen, 'wlen': self.wlen}
    ...
    def gcnLayer(self, gcn_in, in_dim, gcn_dim, batch_size, max_nodes, max_labels, adj_mat,
        ...
        for layer in range(num_layers):
            gcn_in = out[-1]
            if len(out) > 1:
                in_dim = gcn_dim         # After first iteration the in_dim = gcn_dim
                ...
                if layer > 0 and self.p.loop:
                    with tf.variable_scope('Loop-name-%s_layer-%d' % (name, layer)) as scope:
                        ...
                        if self.p.dropout != 1.0:
                            inp_loop = tf.nn.dropout(inp_loop, keep_prob=self.p.dropout)
                        if w_gating:
                ...
                for lbl in range(max_labels):
                    with tf.variable_scope('label-%d_name-%s_layer-%d' % (lbl, name, layer)) as scope:
                        ...
                        w_out = tf.get_variable('w_out', [in_dim, gcn_dim],
                                                initializer=tf.contrib.layers.xavier_initializer(),
                                                regularizer=self.regularizer)
                        b_in = tf.get_variable('b_in', [1, gcn_dim],
                                               initializer=tf.constant_initializer(0.0),
                                               regularizer=self.regularizer)
                        b_out = tf.get_variable('b_out', [1, gcn_dim],
                                                initializer=tf.constant_initializer(0.0),
                                                regularizer=self.regularizer)
                        if w_gating:
                            w_gin = tf.get_variable('w_gin', [in_dim, 1],
                                                    initializer=tf.contrib.layers.xavier_initializer(),
                                                    regularizer=self.regularizer)
                            b_gin = tf.get_variable('b_gin', [1],
                                                    initializer=tf.constant_initializer(0.0),
                                                    regularizer=self.regularizer)
                            w_gout = tf.get_variable('w_gout', [in_dim, 1],
                                                     initializer=tf.contrib.layers.xavier_initializer(),
                                                     regularizer=self.regularizer)
                            b_gout = tf.get_variable('b_gout', [1],
                                                     initializer=tf.constant_initializer(0.0),
                                                     regularizer=self.regularizer)
    ...
    def add_model(self):
        ...
        with tf.variable_scope('Embed_mat'):
            ...
            if self.p.embed_loc:
                embed_init = hp.getEmbeddings(self.p.embed_loc,
                                              [self.id2voc[i] for i in range(len(self.voc2id))],
                                              self.p.embed_dim)
    ...
    def add_loss_op(self, nn_out):
        ...
        neg_ids, _, _ = tf.nn.fixed_unigram_candidate_sampler(
                        ...
                        unique=True, distortion=0.75, range_max=self.vocab_size,
    ...
    def run_epoch(self, sess, epoch, shuffle=True):
        ...
        for step, batch in enumerate(self.getBatches(shuffle)):
            ...
            if (step+1) % 10 == 0:
                self.logger.info('E:{} (Sents: {}/{} [{}]): Train Loss \t{:.5}\t{}\t{:.5}'.format(epoch, cnt,
                                 self.p.total_sents, round(cnt/self.p.total_sents * 100, 1), np.mean(losses),
...
if __name__ == "__main__":
    ...
    # Added these two arguments to enable others to personalize the training set.
    # Otherwise, the programme may suffer from memory overflow easily.
