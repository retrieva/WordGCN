...
class SemGCN(Model):
    def make_batch(self, shuffle=True):
        ...
        for semantic in self.semantic_list:
            for line in open('./{}/{}.txt'.format(self.p.semdir, semantic), encoding='utf-8', errors='ignore'):
                ...
                ele['Edges'] = [[i, j, self.lbl2id[semantic]]
                                for i, j in itertools.permutations(range(len(ele['Words'])), 2)]
                ele['Cur_len'] = len(ele['Words'])
                ele['Edges'] = [[e[0], e[1], e[2]]
                                for e in ele['Edges'] if ele['Words'][e[0]] != 0 and ele['Words'][e[1]] != 0]
                batch.append(ele)
    ...
    def load_data(self):
        ...
        self.rej_prob = {_id: (1 - self.p.subsample / rel_freq[_id]) - np.sqrt(self.p.subsample / rel_freq[_id])
            ...
            with tf.name_scope('%s-%d' % (name, layer)):
                ...
                for lbl in range(max_labels):
                    with tf.variable_scope('label-%d_name-%s_layer-%d' % (lbl, name, layer)) as scope:
                        w_in = tf.get_variable('w_in', initializer=tf.eye(in_dim),
                                               trainable=True, regularizer=self.regularizer)
                        w_out = tf.get_variable('w_out', initializer=tf.eye(in_dim),
                                                trainable=True, regularizer=self.regularizer)
                    ...
                    with tf.name_scope('out_arcs-%s_name-%s_layer-%d' % (lbl, name, layer)):
                        ...
                        if w_gating:
                            ...
                            # out_act = out_t
                            out_act = self.aggregate(out_act, adj_matrix)
    ...
    def add_model(self):
        ...
        with tf.variable_scope('Embed_mat'):
            embed_init = hp.getEmbeddings(self.p.embed_loc, [self.id2voc[i] for i in range(len(self.voc2id))],
                                          self.p.embed_dim)
        ...
        for step, batch in enumerate(self.getBatches(shuffle)):
            ...
            if (step+1) % 10 == 0:
                self.logger.info('E:{} (Sents: {}/{} [{}]): Train Loss \t{:.5}\t{}\t{:.5}'.format(epoch, self.sent_num,
                                                                                                  self.p.total_sents,
                                                                                                  round(self.sent_num / self.p.total_sents * 100, 1),
                                                                                                  np.mean(losses),
                                                                                                  self.p.name,
                                                                                                  self.best_int_avg))
                en = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-semdir', dest="sem_dir", default='./semantic_info', help='semantic file directory')
